...
import ai.classifai.util.async.JsonStoreUtil;
...
public class ProjectController extends AbstractVertxController
{
    ...
    public static final String PATH_VALID_KEY = "path_valid";
    public static final String PROJECT_LOADED_KEY = "project_loaded";
    ...
    ImageDataVersionService imageDataVersionService;

    ProjectLoadingService projectLoadingService;

    public ProjectController(Vertx vertx, DbService dbService, ProjectService projectService, DataService dataService,
                             ImageService imageService, LabelService labelService,
                             ImageDataVersionService imageDataVersionService, ProjectLoadingService projectLoadingService)
    {
        super(vertx);
        ...
        this.imageDataVersionService = imageDataVersionService;
    ...
    public void loadProject(RoutingContext context)
    {
        ...
        Future<List<Data>> validDataListFuture = projectFuture
                ...
                .compose(dataList -> dataService.filterValidData(dataList));
    ...
    public void deleteProject(RoutingContext context)
    {
        // get project
        // delete project
        // TODO: implementation time, delete all stuff that is related to the project
        String projectName = paramHandler.getProjectName(context);
        AnnotationType annotationType = paramHandler.getAnnotationType(context);

        dbService.getProjectByNameAndAnnotation(projectName, annotationType)
                .compose(project -> dbService.deleteProjectFuture(project))
    ...
    public void starProject(RoutingContext context)
    {
        ...
        context.request().bodyHandler(buffer ->
        {
            ...
            dbService.getProjectByNameAndAnnotation(projectName, annotationType)
                    .compose(project -> dbService.starProjectFuture(project, isStarred))
                    .onSuccess(unused -> sendEmptyResponse(context))
                    .onFailure(failedRequestHandler(context));
...
//    {
//        // get project
//        // TODO: need to check is project in list?
//        // remove from loaded list
//        String projectName = paramHandler.getProjectName(context);
//        AnnotationType annotationType = paramHandler.getAnnotationType(context);
//
//        getProjectByNameAndAnnotation(annotationType, projectName)
...
//        AnnotationType annotationType = paramHandler.getAnnotationType(context);
//
//        Future<Project> getProjectFuture = getProjectByNameAndAnnotation(annotationType, projectName);
//
...
//
//        getDataListFuture
//                .compose(dataList -> imageService.getToAddDataDtoList(dataList, getProjectFuture.result()))
//                .compose(this::addDataDTOList)
//                .onSuccess(addedDataList ->
//                        {
//                            List<DataDTO> dataDTOList = addedDataList.stream()
//                                    .map(Data::toDTO)
//                                    .collect(Collectors.toList());
//
//                            sendResponseBody(ProjectReloadView.generate(dataDTOList), context);
//                        })
//                .onFailure(failedRequestHandler(context));
//    }
     ...
     *
     */
    public void createImageProject(RoutingContext context)
    {
        context.request().bodyHandler(buffer ->
        {
            ...
            Future<List<ImageDataDTO>> dataDTOListFuture = imageService.getImageDTOList(projectPath);
            ...
            CompositeFuture.all(dataDTOListFuture, labelDTOListFuture, projectAnnotationAndNameAvailableFuture)
                    .compose(unused ->
                    {
                        List<ImageDataDTO> dataDTOS = dataDTOListFuture.result();
                        List<LabelDTO> labelDTOS = labelDTOListFuture.result();

                        return dbService.createImageProject(projectDTO, versionDTO, dataDTOS, labelDTOS);
                    })
                    .onSuccess(unused -> sendEmptyResponse(context))
                    .onFailure(failedRequestHandler(context));
        });
    ...
    public static Future<Void> storeProject(Project project, JsonObject store)
    {
        return JsonStoreUtil.storeItem(project, store, PROJECT_KEY);
    }

    public static Future<Void> storePathValid(Boolean isPathValid, JsonObject store)
    {
        return JsonStoreUtil.storeItem(isPathValid, store, PATH_VALID_KEY);
    }

    public static Future<Void> storeProjectLoaded(Boolean isProjectLoaded, JsonObject store)
    {
        return JsonStoreUtil.storeItem(isProjectLoaded, store, PROJECT_LOADED_KEY);
    }

    public static Project getProjectFromStore(JsonObject store)
    {
        return (Project) store.getValue(PROJECT_KEY);
    }

    public static Boolean getPathValidFromStore(JsonObject store)
    {
        return store.getBoolean(PATH_VALID_KEY);
    }

    public static Boolean getProjectLoadedFromStore(JsonObject store)
    {
        return store.getBoolean(PROJECT_LOADED_KEY);
    }

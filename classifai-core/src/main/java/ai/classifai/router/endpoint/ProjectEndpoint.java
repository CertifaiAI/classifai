...
public class ProjectEndpoint {
    ...
    @PUT
    @Path("/v2/imagefiles")
    public ActionStatus selectImageFile()
    {

        if(!ui.isImageFileSelectorOpen())
        {
            ui.showImageFileSelector();
    ...
    @GET
    @Path("/v2/imagefiles")
    public SelectionStatusResponse selectImageFileStatus() {

        SelectionWindowStatus status = ui.getImageFileSelectorWindowStatus();
        ...
        if (status.equals(SelectionWindowStatus.WINDOW_CLOSE)) {
            response.setImagePathList(ui.getImagePathList());
            response.setImageDirectoryList(ui.getImageDirectoryList());
        }

...
# ----------------------------------- Deploy Classifai into Docker -----------------------------------------------------

# Step 1: Move this docker file into classifai-uberjar/target
#
# Step 2: Change the ENV parameters below to different name or path accordingly. Remove the ENV parameter that not required
...
# For Import project: ENV parameter required are port, datapath, configpath
#
# ----------------------------------------------------------------------------------------------------------------------
...
CMD ["sh", "-c", "java -jar classifai-uberjar-dev.jar --unlockdb --docker --port=$port --projectname=$projectname --projecttype=$projecttype --datapath=$datapath --labelpath=$labelpath --configpath=$configpath"]